...
import { VISA_PAGES } from "./content/visa-pages.js";
 ...
 *
 * Global Visa sits straight after Services and took the place Campaigns had
 * in the bar, at the client's instruction: visa files are core work and
 * deserve a menu of their own. /campaigns/ still exists and is still linked
 * from the footer and the homepage; it is only the menu entry that went.
 *
 * The Global Visa items carry a `description` and `flags`, which the header
 * shows in its dropdown. Their hrefs come from VISA_PAGES in
 * lib/content/visa-pages.js, so the pages and the menu cannot disagree.
 */
export const NAV = [
  {
    label: "Destinations",
    ...
    columns: [
      {
        label: "Visa services",
        href: VISA_PAGES.hub.href,
        items: ["tourist", "dependant", "settlement"].map((key) => ({
          label: VISA_PAGES[key].navLabel,
          href: VISA_PAGES[key].href,
          description: VISA_PAGES[key].summary,
          flags: VISA_PAGES[key].flags,
        })),
      },
    ],
  },
